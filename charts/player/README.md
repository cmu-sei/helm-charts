...
| `Vsphere__Hosts__*__IsoStorageGroup` | Optional ISO destination group; overrides `Vsphere__IsoStorageShared`. Members must reach the same physical ISO directory | Unset |
...
| `Vsphere__IsoStorageShared` | In API mode, ungrouped vCenters share one ISO destination, with fallback between connected members | `false` |
...
In API mode the ISO is pushed **once per ISO destination**, using a connected member’s own `DsName` and
...
Cancellation stops waiting for SOAP inventory lookups and directory creation, but does
not stop calls already running in vCenter. A directory can be created after the request is cancelled.

Configure each vCenter address only once. Duplicate addresses are unsupported.
