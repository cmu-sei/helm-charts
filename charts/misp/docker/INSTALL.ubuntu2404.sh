...
apt-get update && apt-get install -y sudo openssl lsb-release ca-certificates
...
sudo cp /tmp/trust-store.pem /usr/local/share/ca-certificates/trust-store.crt &>> $logfile
...
# curl --cacert /tmp/trust-store.pem -sS https://getcomposer.org/installer -o /tmp/composer-setup.php &>> $logfile
curl -sS https://getcomposer.org/installer -o /tmp/composer-setup.php &>> $logfile
# COMPOSER_HASH=`curl --cacert /tmp/trust-store.pem -sS https://composer.github.io/installer.sig`
...
# sudo php /tmp/composer-setup.php --cafile /tmp/trust-store.pem --install-dir=/usr/local/bin --filename=composer  &>> $logfile
sudo php /tmp/composer-setup.php --install-dir=/usr/local/bin --filename=composer  &>> $logfile
error_check "Composer installation"

#sudo -u ${APACHE_USER} composer config -g secure-http false &>> $logfile
#sudo -u ${APACHE_USER} composer config -g -- disable-tls true &>> $logfile
...
#sudo git config --global http.sslCAinfo /tmp/trust-store.pem &>> $logfile
...
sudo -u ${APACHE_USER} gpg --homedir ${MISP_PATH}/.gnupg --batch --generate-key /tmp/gpg-config  &>> $logfile
...
sudo service supervisor start &>> $logfile
